...
void track_session(DataBase* dataBase){
    if(ImGui::Begin("Track Session", NULL, ImGuiWindowFlags_AlwaysAutoResize)){
        ...
        if(ImGui::CollapsingHeader("LAPS")){
            int maxNumLap = dataBase->telemetryData.telemetryDataPerLap.size();
            ImGui::Text("SELECT LAP");
            ImGui::SliderInt("", &currentLap, 1, maxNumLap);
            ImGui::Text("CURRENT LAP %d", currentLap);
            int maxSizeData = dataBase->telemetryData.telemetryDataPerLap[currentLap-1].size();
            float velocityArr[maxSizeData];
            float timeArr[maxSizeData];
            float averageVelocity = 0;
            float finalTimeLap = 0;
            for(int i = 0; i < maxSizeData; i++){
                velocityArr[i] = dataBase->telemetryData.telemetryDataPerLap[currentLap-1][i].getSpeed();
                timeArr[i] = dataBase->telemetryData.telemetryDataPerLap[currentLap-1][i].getlapTime();
                averageVelocity += velocityArr[i];
            }
            finalTimeLap = dataBase->telemetryData.telemetryDataPerLap[currentLap-1][maxSizeData-1].getlapTime();
